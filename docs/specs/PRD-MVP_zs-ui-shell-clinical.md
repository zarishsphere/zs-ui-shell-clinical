...
Clinical app shell — composes all clinical MFEs. is required for shell clinical workflows in the ZarishSphere platform.
...
- As a user, I can use clinical app shell — composes all clinical mfes. through a clean, accessible interface.
...
|----|------------|---------------------|---------|
| M-01 | Clinical app shell — composes all clinical MFEs. renders without error | Main component loads in browser | P0 |
...
│   ├── hooks/
│   │   ├── useShellclinicalData.ts  # FHIR data hook
│   │   └── useOfflineSync.ts              # Dexie.js sync hook
│   ├── pages/
│   │   ├── index.tsx                      # Main page
│   │   └── (feature pages)
│   ├── store/
│   │   └── shell_clinicalStore.ts                  # Zustand state slice
│   ├── types/
...
│   │   └── components/
│   └── e2e/
│       └── shell_clinical.spec.ts                  # Playwright E2E test
└── public/
    └── index.html
